...
<div class="flex flex-col items-center justify-center min-h-screen py-6 sm:py-12 lg:py-24 bg-white">
    ...
    <form action="{{ route('login.store') }}" method="POST" class="w-full max-w-xs flex flex-col gap-6">
        ...
        <div class="relative mt-2">
            <label for="email" class="absolute -top-3 left-6 bg-white px-1 text-base font-jost">Usuário</label>
            <input type="text" id="user" name="email" required
                class="w-full px-4 py-2 border border-black rounded-full focus:outline-none focus:ring-2 focus:ring-blue-800 text-base font-jost" />
        </div>
        <div class="relative mt-2">
            <label for="password" class="absolute -top-3 left-6 bg-white px-1 text-base font-jost">Senha</label>
    ...
    <div class="mt-6 text-center">
        <span class="text-base font-jost">Esqueceu a senha? </span>
